        ...
        for (int i = 0; i < TREE_NODES.size(); i++) {
            ...
            if ("1".equals(thisMsg.get("RET_CODE") + "")) {
                ...
                /*if(fileSuffix.equals(".ofd")){
                    newFilePath = filePath.substring(0, len)+".ofd";
                    syntheticType = "ofd";
                }*/
                log.info("日志1---------");
                ...
                //}else if(fileSuffix.equals(".doc")){//wordtoPDF
                ...
                /*int len11 = fileNo.lastIndexOf(".");
                String fileNameType = fileNo.substring(len11);*/
                String savePath = this.filePath + "/" + fileNo ;
                /*if(fileNameType.equals(".ofd")){
                    newFilePath = filePath.substring(0, len)+".ofd";
                    syntheticType = "ofd";
                }*/
                ...
                for (int i = 0; i < TREE_NODES.size(); i++) {
                    ...
                    if ("1".equals(thisMsg.get("RET_CODE") + "")) {
                        ...
                        if (fileSuffix.equals(".doc") || fileSuffix.equals(".docx")) {//wordtoPDF
                            wordToPdf(TREE_NODES.get(i), filePath1, newFilePath);
                            int nObjID = srvSealUtil.openObj(newFilePath, 0, 0);
                            //int nObjID = srvSealUtil.openObj(newFilePath, 0, 0);
                            if (nObjID <= 0) {
                                /*try {
                                    throw new Exception("文件转换异常");
                                ...
                                return getReturnXml(null, "", beginTime,null, "文件转换异常");
                               // return getReturnXml(null, "", beginTime,null, "文件转换异常");
  ...
  /*  private Map sealAutoPdfForNotMerge(Element sealDocRequest, SyntheticPattern syntheticPattern, String beginTime) throws Exception {
    ...
    public String pdfVarify(String xmlStr, String beginTime, HttpServletRequest request) {
        try {
            ...
            xmlStr = parvifyXml(xmlStr,beginTime);
            Document doc = DocumentHelper.parseText(xmlStr);
            ...
            }else if(FILE_TYPE!=null&&FILE_TYPE.equals("2")){
                FileDownRet="ok";
                filePaths.put("fileUrl",FILE_PATH);
                filePaths.put("fileName","");
                filePaths.put("fileSuffix",".pdf");
            }else{
...
//  			System.out.println("Linux");
    ...
    //重组xml
    public   String  parvifyXml(String   xml,String beginTime){

        //base  还原为文件
        String   newXml = xml;
        try {
            if(xml!=null&&!"".equals(xml)&&xml.contains("<FILE_DATA>")){
                //提取<FILE_DATA>标签内的数据
                Document doc = DocumentHelper.parseText(xml);
                Element verifyDocRequest = doc.getRootElement();
                Element META_DATA = verifyDocRequest.element("META_DATA");
                String   base64document = META_DATA.elementText("FILE_DATA").trim();
                //String     base64document1 = base64document.replaceAll(" ","+");
                String fname=new Date().getTime()+"";
                fname=beginTime+"-"+fname;
                /*String   fname1 = "seal"+UUIDReduce.uuid()+".pdf";*/
                String   realLocal = Util.getSystemDictionary("upload_path")+"/downPathHttp/"+fname+".pdf";
                log.info(base64document);
                Util.base64ToFile(base64document,realLocal);
                //将xml中的<FILE_DATA>  改造xml
               // String   meta_data= "<FILE_NO>"+fname+".pdf</FILE_NO>\n" +
                        String   meta_data= "<FILE_NO>"+fname+".pdf</FILE_NO>\n" +
                        "<FILE_PATH>"+realLocal+"</FILE_PATH>\n";
               newXml = xml.replace("<FILE_DATA>"+base64document+"</FILE_DATA>",meta_data);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
